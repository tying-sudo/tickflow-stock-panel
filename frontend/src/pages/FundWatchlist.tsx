...
function StockSearchBox({
  ...
  /** 基金页差异: ETF 添加走建成分股组 (组名自动生成, 无需选分组) */
  onAddEtf: (symbol: string) => void
              ...
              return (
                ...
                >
                  ...
                  {inWatchlist ? (
                    ...
                    // 未加自选: ETF → 一键建成分股组 (组名=基金名, 无需选分组);
                    // 个股 → + 加入当前分组页签 / 文件夹图标展开分组菜单
                    <span className="flex shrink-0 items-center gap-0.5">
                      {r.asset_type === 'etf' ? (
                        <button
                          type="button"
                          onClick={event => { event.stopPropagation(); onAddEtf(r.symbol) }}
                          disabled={addPending}
                          className="shrink-0 rounded p-1 text-accent transition-colors hover:bg-accent/10 disabled:opacity-50 cursor-pointer"
                          title={`以「${r.name}」建成分股组 (自动按基金名建组, 成分股入组)`}
                          aria-label={`建成分股组 ${r.symbol}`}
                        >
                          <FolderPlus className="h-3.5 w-3.5" />
                        </button>
                      ) : (
...
export function FundWatchlist() {
  ...
  // 基金页差异: ETF 添加 = add-etf-group 建成分股组 (按基金名建组, 成分股入组)
        ...
        right={
          <div className="flex items-center gap-2">
            ...
            <StockSearchBox
              ...
              onAddEtf={symbol => addEtfGroupMutation.mutate(symbol)}
              onToggleMember={handleToggleMember}
              preferredGroupId={activeGroupId}
              addPending={addMutation.isPending || addEtfGroupMutation.isPending}
