...
> Git：本地完整状态已推 `github.com/tying-sudo/tickflow-stock-panel` 分支 `deploy/v0.2.2-container`；**目标仓库 main = 上游合并分支，用户明令禁止覆盖**。
...
- 推送前本地分支 `deploy/v0.2.2-container` = `2fb9c96`（V2 交接文档），与远端同步。
- **本工程 3 个新提交**（本档随同一并推送）：
